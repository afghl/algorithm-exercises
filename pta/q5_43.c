// 给定一系列由大写英文字母组成的字符串关键字和素数PP，用移位法定义的散列函数H(Key)H(Key)将关键字KeyKey中的最后3个字符映射为整数，每个字符占5位；再用除留余数法将整数映射到长度为PP的散列表中。例如将字符串AZDEG插入长度为1009的散列表中，我们首先将26个大写英文字母顺序映射到整数0~25；再通过移位将其映射为3\times 32^2 + 4 \times 32 + 6 = 32063×32
// ​2
...
// 输入格式:
//
// 输入第一行首先给出两个正整数NN（\le 500≤500）和PP（\ge 2N≥2N的最小素数），分别为待插入的关键字总数、以及散列表的长度。第二行给出NN个字符串关键字，每个长度不超过8位，其间以空格分隔。
//
// 输出格式:
//
